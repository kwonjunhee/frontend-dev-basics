...
window.addEventListener("DOMContentLoaded", function() {
	document
		...
		} else if(window.XMLHttpRequest) { // XMLHttpRequest를 생성한거임
		...
		xhr.addEventListener('readystatechange', function() {
			if(this.readyState == XMLHttpRequest.UNSENT) { // readyStete: 0
				// request가 초기화되기 전
				console.log('State: UNSENT');
					
			} else if(this.readyState == XMLHttpRequest.OPENED) { // readyStete: 1
				// 서버와 연결이 성공
				console.log('State: OPENED');
					
			} else if(this.readyState == XMLHttpRequest.HEADERS_RECEIVED) { // readyStete: 2
				// 서버가 request를 받음
				console.log('State: HEADERS_RECEIVED');
					
			} else if(this.readyState == XMLHttpRequest.LOADING) { // readyStete: 3
				// 서버가 response를 처리 중 (response body 파싱 중)
				console.log('State: LOADING');
				
			} else if(this.readyState == XMLHttpRequest.DONE) { // readyStete: 4
				...
				// var response = JSON.parse(this.responseText);
				// console.log(response, typeof(this.response));
			}
		});
